...
namespace Capital.DAL
{
    public class PolicyIssueRepository : BaseRepository
    {
        ...
        public List<PolicyIssue> GetNewPolicy(int type,int Id, string userRolename, DateTime? FromDate, DateTime? ToDate, string PolicyNo = "", string Client = "", string SalesManager = "")
        {
            using (IDbConnection connection = OpenConnection(dataConnection))
            {
                ...
                if (userRolename == "Administrator")
                {
                    query = @"select P.PolicyId,Concat(P.TranPrefix,'/',P.TranNumber)StrTranNumber,C.CusName,P.CustContPersonName,P.InsuredName,I.InsCmpName,IP.InsPrdName,IC.InsCoverName,P.EffectiveDate,P.RenewalDate,
                                 P.PremiumAmount,P.ExtraPremium,P.Totalpremium,P.CommissionAmount, S.SalesMgName,P.PolicyNo,P.PolicyStage
                                 from PolicyIssue P
                                 ...
                                 where P.OldPolicyId IS NULL AND P.TranType='NewPolicy' and  P.PolicyStage >=@type
                ...
                else
                {
                    query = @"select SalesMgId into #TEMP from [User]  U  WHERE U.UserId=@Id 
                                 ...
                                 left join InsuranceCoverage IC on IC.InsCoverId = P.InsCoverId
                                 left join SalesManager S on S.SalesMgId = P.SalesMgId
                                 where P.OldPolicyId IS NULL AND P.TranType='NewPolicy' and   isnull(P.SalesMgId,0) IN (SELECT SalesMgId FROM #TEMP) and  P.PolicyStage >=@type
                                 AND cast(convert(varchar(20),P.TranDate,106) as datetime) between @FromDate and @ToDate
                                 AND C.CusName LIKE '%'+@Client+'%'
                                 AND isnull(P.PolicyNo,0) LIKE '%'+@PolicyNo+'%'
                                 AND isnull(S.SalesMgName,0) LIKE '%'+@SalesManager+'%'
                ...
                return connection.Query<PolicyIssue>(query, new { FromDate = FromDate, ToDate = ToDate, PolicyNo = PolicyNo, Client = Client, SalesManager = SalesManager, Id = Id, type = type }).ToList<PolicyIssue>();
            }
        }
...
//                    int id = connection.Execute(sql, model.Cheque);
        ...
        public PolicyIssue GetNewPolicy(int Id)
        {
            using (IDbConnection connection = OpenConnection(dataConnection))
            {
                string sql = @"select PolicyId,(TranPrefix+'/'+TranNumber)TranNumber,TranDate,CusId,InsuredName,Address1,Address2,InsCmpId,InsPrdId,InsCoverId,PolicySubDate,EffectiveDate,RenewalDate,
                                    ...
                                    PaymentOption,SalesMgId,OperationManager,PolicyNo,Remarks,FinanceManager,PaymentTo,PayModeId,OldPolicyId,CIBEffectiveDate,AdditionEmpNo,
                                    DeletionEmpNo,EndorcementTypeId,TranType,OldPolicyNo,OldCompany,OldProductType,OldPremiumAmt,CreatedBy,CreatedDate,ICActualDate,QuickBookRefNo,PolicyStage from PolicyIssue where PolicyId=@Id";
        ...
        public List<PolicyIssue> GetNewPolicyForCommitments(int Id, string userRolename, string trnno = "", string client = "", string insuredname = "", string insuredComp = "", string coverage = "")
        {
            using (IDbConnection connection = OpenConnection(dataConnection))
            {
                string query = "";
                ...
                if (userRolename == "Administrator")
                {
                    ...
                    query = @" select P.PolicyId,Concat(P.TranPrefix,'/',P.TranNumber)StrTranNumber,C.CusName,P.CustContPersonName,P.InsuredName,
                                 ...
                                 left join InsuranceCoverage IC on IC.InsCoverId = P.InsCoverId
                                 left join SalesManager S on S.SalesMgId = P.SalesMgId
                                 where P.OldPolicyId IS NULL and P.PolicyStage = 1  
                ...
                else
                {
                    query = @"select SalesMgId into #TEMP from [User]  U  WHERE U.UserId=@Id 
                                 ...
                                 left join InsuranceCoverage IC on IC.InsCoverId = P.InsCoverId
                                 left join SalesManager S on S.SalesMgId = P.SalesMgId
                                 where P.OldPolicyId IS NULL and P.PolicyStage = 1  and  isnull(P.SalesMgId,0) IN (SELECT SalesMgId FROM #TEMP)
                                 and p.TranNumber like'%'+@trnno+'%'
                                 and c.CusName like '%'+@Client+'%'
								 and p.InsuredName like '%'+@insuredname+'%'
								 and I.InsCmpName like '%'+@insuredComp+'%'
        ...
        public List<PolicyIssue> GetNewPolicyForPaymentCollection(string trnno = "", string client = "", string insuredname = "", string insuredComp = "", string coverage = "")
        {
            using (IDbConnection connection = OpenConnection(dataConnection))
            {
                ...
                string query = @"  select P.PolicyId,Concat(P.TranPrefix,'/',P.TranNumber)StrTranNumber,C.CusName,P.CustContPersonName,P.InsuredName,I.InsCmpName,
                                    IP.InsPrdName,IC.InsCoverName,P.EffectiveDate,P.RenewalDate,P.PolicyStage,
                                    ...
                                    where P.OldPolicyId IS NULL AND P.TranType in ('NewPolicy','RenewPolicy') and PolicyStage in(2,3)
                                    ...
                                    P.PremiumAmount,P.ExtraPremium,P.Totalpremium,P.CommissionAmount, S.SalesMgName,P.PolicyNo,p.TranPrefix,p.TranNumber,P.PolicyStage
        ...
        public IEnumerable<PolicyIssueChequeReceived> GetReceiptChequeDetailsforPrint(int Id)
        {
            using (IDbConnection connection = OpenConnection(dataConnection))
            {
                ...
                sql = @"select  PM.PayModeName,R.ChequeAmt TotalPremium,isnull(R.ChequeNo,'')ChequeNo,isnull(R.ChequeDate,'')ChequeDate,isnull(R.BankName,'')BankName,C.CusName  from  PolicyIssue P
